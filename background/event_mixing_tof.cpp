...
int main(int argc, char **argv) {
  if (argc != 4) {
    cerr << "Wrong number of arguments. Instead use\n"
         << "\t./event_mixing [outputRootfile] [inputElectronSkim] "
            "[inputTaggedSkim]\n";
    return -1;
  }
  ...
  // Define 4D binned ToF plots and bin edges and kin cuts from kinematic_cuts.h
  // file
  const double max_CosTheta_nq =
      cos(NCUT_THETANQ_min); // bit non-intuitive but min angle = 150 which is a
                             // "max" costhetanq of -0.8
  const double min_CosTheta_nq =
      cos(NCUT_THETANQ_max); // and then max angle = 180 which is a "min"
                             // costhetanq of -1
  ...
  // Read entire tree into memory due to issue with jumping around in tree as we
  // read it..
  ...
  for (int neutron = 0; neutron < inTree_n->GetEntries(); neutron++) {
    ...
    // Check that the neutron we have is in our background region, in the
    // CosThetaNQ bin, and in the Q2 bin
    if (lead_n->getTof() < NCUT_BACK_Tof_min ||
        lead_n->getTof() > NCUT_BACK_Tof_max)
      continue;
  ...
  for (int neutron = 0; neutron < neutron_list.size(); neutron++) {
    if (neutron % 1000 == 0)
      cout << "working on neutron " << neutron << "\n";
    ...
    TVector3 eVec;
    eVec.SetMagThetaPhi(p_e, theta_e, phi_e);
    TVector3 qVec;
    qVec = beamVec - eVec;
    ...
    for (int thisBunch = 0; thisBunch < (signal_max - signal_min) / BEAM_BUNCH;
         thisBunch++) {
      neutrons_created++;

      // Shift ToF by current background bunch time and desired signal bunch
      // time
      double bunch_shift = (signal_min + BEAM_BUNCH * thisBunch) -
                           (this_bBunch * BEAM_BUNCH + bkgrd_min);
      ...
      if (direction.Z() >
          0) { // this means the phi_rq should be between 0 and pi
      } else if (direction.Z() <
                 0) { // this means the phi_rq should be between -pi and 0
        phi_nq *= (-1);
      }
      double W_primeSq = mD * mD - Q2 + mN * mN + 2. * mD * (nu - E_n) -
                         2. * nu * E_n + 2. * q * p_n * cos(theta_nq);
      ...
      if (CosTheta_nq > min_CosTheta_nq && CosTheta_nq < max_CosTheta_nq) {
