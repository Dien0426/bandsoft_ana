...
void yn_sector(TString inDat, TString inBac, TString inSim) {
  ...
  c0->Print("Yn_sector_Dat_MC_check.pdf[");
  TCanvas *c00 = new TCanvas("c00");
  c00->Print("Yn_sector_Dat_BG_check.pdf[");

  TCanvas *c000 = new TCanvas("c000");
  c000->Print("Yn_Csim_layer_sector.pdf[");
  ...
  for (int l = 1; l < N_layer + 1; l++) {
    ...
    for (int s = 1; s < N_sector + 1; s++) {
      ...
      TH1D *h1_dat = new TH1D(Form("h1_dat_l%d_s%d", l, s), "", 300, -50, 100);

      TH1D *h1_BG = new TH1D(Form("h1_BG_l%d_s%d", l, s), "", 300, -50, 100);

      h1_BG->SetLineColor(2);
      TH1D *h1_MC = new TH1D(Form("h1_MC_l%d_s%d", l, s), "", 300, -50, 100);
      ...
      // inTreeDat->Draw(
      //      Form("nHits[nleadindex]->getDL().Y() >> h1_dat_l%d_s%d", l, s),
      //      cut,
      //     "");

      inTreeDat->Draw(Form("nHits[nleadindex]->getY() >> h1_dat_l%d_s%d", l, s), cut && edep_cut, "");

      h1_dat->SetTitle(title0);
      h1_dat->GetXaxis()->SetTitle("y [cm]");
      h1_dat->GetYaxis()->SetTitle("Count");

      // inTreeBac->Draw(
      //    Form("nHits[nleadindex]->getDL().Y() >> h1_BG_l%d_s%d", l, s), cut,
      //    "same hist");

      inTreeBac->Draw(Form("nHits[nleadindex]->getY() >> h1_BG_l%d_s%d", l, s), cut && edep_cut, "same hist");

      if (h1_dat->GetEntries() > 0) {

        cc->Print("Yn_sector_Dat_BG_check.pdf");
      }
      ...
      if (h1_dat->GetEntries() > 0 && h1_BG->GetEntries() > 0) {
        ...
        h1_dat->Add(h1_BG, -1);
        // inTreeSim->Draw(
        //    Form("nHits[nleadindex]->getDL().Y() >> h1_MC_l%d_s%d", l, s),
        //    cut,
        //    "");

        inTreeSim->Draw(Form("nHits[nleadindex]->getY() >> h1_MC_l%d_s%d", l, s), cut && edep_cut, "");
        ...
        label1D(h1_dat, h1_MC, "y [cm]", "Counts");
        c->Print("Yn_sector_Dat_MC_check.pdf");
      }
      //}
    }
    TCanvas *c_sim = new TCanvas(Form("c_sim_%d", l), "", 800, 600);
    h1_csim_layer->Draw("hist");
    c_sim->Print("Yn_Csim_layer_sector.pdf");
  }
