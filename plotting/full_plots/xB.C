...
void xB(TString inDat, TString inBac, TString inSim){
	...
	TH1D ** xB_bac = new TH1D*[3];
	TH1D ** xB_sim = new TH1D*[3];
	for(int i = 0 ; i < 3 ; i++){
		xB_dat[i] = new TH1D(Form("xB_dat_%i",i),"",20,0.1,0.5);
		xB_bac[i] = new TH1D(Form("xB_bac_%i",i),"",20,0.1,0.5);
		xB_sim[i] = new TH1D(Form("xB_sim_%i",i),"",20,0.1,0.5);
	}
	...
	for( int i = 0 ; i < 3 ; i++){
		...
		c_xB->cd(i+1);
		inTreeDat->Draw(Form("eHit->getXb() >> xB_dat_%i",i),"tag[nleadindex]->getMomentumN().Mag() > 0.3" && pTcut && edep_cut);
		inTreeBac->Draw(Form("eHit->getXb() >> xB_bac_%i",i),"tag[nleadindex]->getMomentumN().Mag() > 0.3" && pTcut && edep_cut);
		inTreeSim->Draw(Form("eHit->getXb() >> xB_sim_%i",i),"tag[nleadindex]->getMomentumN().Mag() > 0.3" && pTcut && edep_cut);

		// Background subraction
		xB_dat[i]->Add(xB_bac[i],-1);
		...
		xB_sim[i]->SetTitle(pTtitle+Form(", C_{sim} = %f, ",sim_scaling));
		label1D(xB_dat[i],xB_sim[i],"x_{B}","Counts");

		c_xB->cd(4+i);
		label1D_ratio(xB_dat[i],xB_sim[i],"x_{B}","Data/Sim",0,2);
	}
