...
void phinq(TString inDat, TString inBac, TString inSim){
	...
	TH1D ** phinq_bac = new TH1D*[3];
	TH1D ** phinq_sim = new TH1D*[3];
	for(int i = 0 ; i < 3 ; i++){
		phinq_dat[i] = new TH1D(Form("phinq_dat_%i",i),"",45,-180,180);
	...
	for( int i = 0 ; i < 3 ; i++){
		...
		double full_simnorm = (double)phinq_dat[0]->Integral() / phinq_sim[0]->Integral();
		if( i == 0 ) sim_scaling = full_simnorm;
		phinq_sim[i]->Scale( sim_scaling );
