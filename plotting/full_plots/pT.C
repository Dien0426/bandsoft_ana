...
void pT(TString inDat, TString inBac, TString inSim){
	...
	for( int i = 0 ; i < 1 ; i++){
		...
		c_pT->cd(i+1);
		inTreeDat->Draw(Form("tag[nleadindex]->getPt().Mag() >> pT_dat_%i",i),"tag[nleadindex]->getMomentumN().Mag() > 0.3" && pTcut && edep_cut);
		inTreeBac->Draw(Form("tag[nleadindex]->getPt().Mag() >> pT_bac_%i",i),"tag[nleadindex]->getMomentumN().Mag() > 0.3" && pTcut && edep_cut);
		inTreeSim->Draw(Form("tag[nleadindex]->getPt().Mag() >> pT_sim_%i",i),"tag[nleadindex]->getMomentumN().Mag() > 0.3" && pTcut && edep_cut);

		// Background subraction
		pT_dat[i]->Add(pT_bac[i],-1);

		// Simulation scaling only from no pT cut distribution (i.e. from full distribution)
		double full_simnorm = (double)pT_dat[0]->Integral() / pT_sim[0]->Integral();
		if( i == 0 ) sim_scaling = full_simnorm;
		pT_sim[i]->Scale( sim_scaling );
