...
void xn_bar(TString inDat, TString inBac, TString inSim) {
  ...
  c0->Print("Xn_bar_Dat_MC_check.pdf[");
  TCanvas *c00 = new TCanvas("c00");
  c00->Print("Xn_bar_Dat_BG_check.pdf[");

  TCanvas *c000 = new TCanvas("c000");
  c000->Print("Xn_Csim_layer.pdf[");
  ...
  for (int l = 1; l < N_layer + 1; l++) {
    ...
    for (int s = 1; s < N_sector + 1; s++) {
      for (int b = 1; b < N_bar; b++) {
        ...
        TH1D *h1_dat =
            new TH1D(Form("h1_dat_l%d_s%d_b%d", l, s, b), "", 25, -125, 125);

        TH1D *h1_BG =
            new TH1D(Form("h1_BG_l%d_s%d_b%d", l, s, b), "", 25, -125, 125);

        h1_BG->SetLineColor(2);
        TH1D *h1_MC =
            new TH1D(Form("h1_MC_l%d_s%d_b%d", l, s, b), "", 25, -125, 125);

        h1_MC->SetLineColor(kGreen);

        TCut cut = Form("nHits[nleadindex]->getLayer() == %d && "
                        "nHits[nleadindex]->getSector()== %d && "
                        "nHits[nleadindex]->getComponent()==%d",
                        l, s, b);

        // Plot Data and BG
        TCanvas *cc =
            new TCanvas(Form("cc_Data_BG_l%d_s%d_b%d", l, s, b), "", 800, 600);
        TString title0 = Form("Layer=%d, Sector=%d, Bar=%d", l, s, b);

        // inTreeDat->Draw(
        //    Form("nHits[nleadindex]->getDL().X() >> h1_dat_l%d_s%d_b%d", l, s,
        //        b),cut, "");
        inTreeDat->Draw(
            Form("nHits[nleadindex]->getX() >> h1_dat_l%d_s%d_b%d", l, s, b),
            cut && edep_cut, "");

        h1_dat->SetTitle(title0);
        h1_dat->GetXaxis()->SetTitle("x [cm]");
        h1_dat->GetYaxis()->SetTitle("Count");

        // inTreeBac->Draw(
        //    Form("nHits[nleadindex]->getDL().X() >> h1_BG_l%d_s%d_b%d", l, s,
        //         b),
        //   cut, "same hist");

        inTreeBac->Draw(
            Form("nHits[nleadindex]->getX() >> h1_BG_l%d_s%d_b%d", l, s, b),
            cut && edep_cut, "same hist");

        if (h1_dat->GetEntries() > 0) {

          cc->Print("Xn_bar_Dat_BG_check.pdf");
        }

        // Background subraction
        if (h1_dat->GetEntries() > 0 && h1_BG->GetEntries() > 0) {
          TCanvas *c =
              new TCanvas(Form("c_Data_MC_l%d_s%d_b%d", l, s, b), "", 800, 600);

          h1_dat->Add(h1_BG, -1);

          // inTreeSim->Draw(
          //     Form("nHits[nleadindex]->getDL().X() >> h1_MC_l%d_s%d_b%d", l,
          //     s,
          //          b),
          //     cut, "");

          inTreeSim->Draw(
              Form("nHits[nleadindex]->getX() >> h1_MC_l%d_s%d_b%d", l, s, b),
              cut && edep_cut, "");

          double scaling = (double)h1_dat->Integral() / h1_MC->Integral();

          h1_csim_layer->Fill(scaling);
          TString title = Form("Layer=%d, Sector=%d, Bar=%d, Csim =%2.4f", l, s,
                               b, scaling);
          h1_MC->Scale(scaling);
          h1_MC->SetTitle(title);
          h1_MC->Draw("hist");
          h1_dat->Draw("same hist");
          label1D(h1_dat, h1_MC, "x [cm]", "Counts");
          c->Print("Xn_bar_Dat_MC_check.pdf");
        }
      }
    }
    TCanvas *c_sim = new TCanvas(Form("c_sim_%d", l), "", 800, 600);
    h1_csim_layer->Draw("hist");
    c_sim->Print("Xn_Csim_layer.pdf");
  }
