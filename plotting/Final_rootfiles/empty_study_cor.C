...
// === Dien 02/15/2021 ======//
//=== I had other mistake in scaling calcalation
//=== the scaling should be Q_tag/Q_empty only
// Since the Al luminosity of LD2 are the same as empty
//==== Dien 02/16/2021 ======//
...
void empty_study_cor() {
  ...
  while (run_empty >> filename) {
    ...
    TFile *Tag_file = new TFile(Form("./skim_tag_file/%s", filename.Data()));
    // TFile *Tag_file = new TFile(Form("./01-28-2021/tag/%s",
    // filename.Data()));
  ...
  // Scaling factor is only ratio of charge
  ...
  TCut tagged_edep =
      inclusive && nGood && nLeadIdx && nStatus && nEdep && nThetaNQ;
  ...
  TCut tagged_signal = tagged_edep && nToF && nPn && nPnNaN && nWp && nAs;
  ...
  c1->Print("empty_study_correct_Q_scale.pdf[");
  c1->Print("empty_study_correct_Q_scale.pdf");
  c2->Print("empty_study_correct_Q_scale.pdf");
  c0->Print("empty_study_correct_Q_scale.pdf");
  c00->Print("empty_study_correct_Q_scale.pdf");
  c00->Print("empty_study_correct_Q_scale.pdf]");
