...
void check_Q_skim() {
  ...
  run_tag.open("./skim_tag_file/run_list.txt");
  run_empty.open("./Empty_run/run_empty.txt");
  ...
  double charge1 = 0;
  double charge2 =0;
  double Q_tot1 = 0, Q_empty1 = 0;
  double Q_tot2 = 0, Q_empty2 = 0;
  ...
  while (run_tag >> filename) {
    ...
    charge1= tag_tree->GetMaximum("gated_charge");
    ...
    cout << N_run << "   " << filename << "  " << charge1 << "   "<< gated_Q << "   "<< gated_Q/charge1<< endl;
  }

  cout << "Number of run: " << N_run << endl;
  cout << "Q_tot1, Q_tot2:  " << Q_tot1<<"   "<< Q_tot2<< "  "<< Q_tot2/Q_tot1<< endl;
  ...
  while (run_empty >> filename) {
    // TFile *Tag_file = new TFile(Form("./01-28-2021-Dien/tag/empty_run/%s", filename.Data()));
     TFile *Tag_file = new TFile(Form("./Empty_run/%s", filename.Data()));
    ...
    cout << N_run_empty << "  " << filename << "  " << charge1<< "  "<< gated_Q<< endl;
  }

  

  cout << "Number of empty run: " << N_run_empty << endl;
  cout << "Q_empty1. Q_empty2:  " << Q_empty1 << "   "<< Q_empty2<< endl;
  ...
  */
  ...
  // TCut tagged = inclusive && nGood && nLeadIdx && nStatus && nEdep &&
  // nThetaNQ;
  ...
  */
  ...
  c1->Divide(2, 1);
  c1->cd(1);
  T_empty->Draw("nHits[nleadindex]->getTof() >> h1_TOF_empty", tagged, "");
  c1->cd(2);
  T_empty->Draw("eHit->getVtz() >> h1_vz_empty", tagged, "");
