void must_plot_layer_new(TString inFileDatTagName, TString inFileBacTagName,
  ...
  TH1D **h1_as_xp_dat = new TH1D
      *[nXp_bins]; // this is an alphaS histogram in a bin of X' for dat
  TH1D **h1_as_xp_bac = new TH1D
      *[nXp_bins]; // this is an alphaS histogram in a bin of X' for dat
  TH1D **h1_as_xp_sim = new TH1D
      *[nXp_bins]; // this is an alphaS histogram in a bin of X' for sim

  TH1D **h1_as_xp_lpt_dat = new TH1D
      *[nXp_bins]; // this is an alphaS histogram in a bin of X' for dat
  TH1D **h1_as_xp_lpt_bac = new TH1D
      *[nXp_bins]; // this is an alphaS histogram in a bin of X' for dat
  TH1D **h1_as_xp_lpt_sim = new TH1D
      *[nXp_bins]; // this is an alphaS histogram in a bin of X' for sim

  TH1D **h1_as_xp_hpt_dat = new TH1D
      *[nXp_bins]; // this is an alphaS histogram in a bin of X' for dat
  TH1D **h1_as_xp_hpt_bac = new TH1D
      *[nXp_bins]; // this is an alphaS histogram in a bin of X' for dat
  TH1D **h1_as_xp_hpt_sim = new TH1D
      *[nXp_bins]; // this is an alphaS histogram in a bin of X' for sim
  ...
  for (int bin = 0; bin < nXp_bins; bin++) {
    h1_as_xp_dat[bin] =
        new TH1D(Form("h1_as_xp_dat_%i", bin), "", nAs_bins, As_min, As_max);
    h1_as_xp_bac[bin] =
        new TH1D(Form("h1_as_xp_bac_%i", bin), "", nAs_bins, As_min, As_max);
    h1_as_xp_sim[bin] =
        new TH1D(Form("h1_as_xp_sim_%i", bin), "", nAs_bins, As_min, As_max);

    h1_as_xp_lpt_dat[bin] = new TH1D(Form("h1_as_xp_lpt_dat_%i", bin), "",
                                     nAs_bins, As_min, As_max);
    h1_as_xp_lpt_bac[bin] = new TH1D(Form("h1_as_xp_lpt_bac_%i", bin), "",
                                     nAs_bins, As_min, As_max);
    h1_as_xp_lpt_sim[bin] = new TH1D(Form("h1_as_xp_lpt_sim_%i", bin), "",
                                     nAs_bins, As_min, As_max);

    h1_as_xp_hpt_dat[bin] = new TH1D(Form("h1_as_xp_hpt_dat_%i", bin), "",
                                     nAs_bins, As_min, As_max);
    h1_as_xp_hpt_bac[bin] = new TH1D(Form("h1_as_xp_hpt_bac_%i", bin), "",
                                     nAs_bins, As_min, As_max);
    h1_as_xp_hpt_sim[bin] = new TH1D(Form("h1_as_xp_hpt_sim_%i", bin), "",
                                     nAs_bins, As_min, As_max);

    h1_as_xp_hpt_03[bin] =
        new TH1D(Form("h1_as_xp_hpt_03_%i", bin), "", nAs_bins, As_min, As_max);
    h1_as_xp_lpt_03[bin] =
        new TH1D(Form("h1_as_xp_lpt_03_%i", bin), "", nAs_bins, As_min, As_max);
    h1_as_xp_03[bin] =
        new TH1D(Form("h1_as_xp_03_%i", bin), "", nAs_bins, As_min, As_max);
  }
  ...
  // First let's get:
  //	R(tag) = 	N_tag,data (Q2,theta_nq,x';alphaS) / [
  // N_tag,sim(Q2,theta_nq,x';alphaS) ] 	as just individual histograms
  ...
  for (int bin = 0; bin < nXp_bins; bin++) {
    ...
    double this_max_xp =
        Xp_min + 0.05 + (bin + 1) * (Xp_max - Xp_min) / nXp_bins;
    TCut this_xp_cut =
  ...
  for (int bin = 0; bin < nXb_bins; bin++) {
    ...
    double this_max_xb =
        Xb_min + 0.05 + (bin + 1) * (Xb_max - Xb_min) / nXb_bins;
    TCut this_xb_cut = Form("eHit->getXb() > %f && eHit->getXb() < %f",
                            this_min_xb, this_max_xb);

    inTreeDatInc->Draw(Form("eHit->getQ2() >> h1_q2_xb_dat_%i", bin),
                       this_xb_cut);
    inTreeSimInc->Draw(Form("eHit->getQ2() >> h1_q2_xb_sim_%i", bin),
                       this_xb_cut);
    ...
    h1_q2_xb_dat[bin]->SetTitle(
        Form("%f < x_{B} < %f", this_min_xb, this_max_xb));
  ...
  for (int bin = 0; bin < nXp_bins; bin++) {
    ...
    label1D_ratio(h1_as_xp_dat[bin], h1_as_xp_sim[bin], "Alpha_{S}",
                  "R=R_{tag}/R_{inc}");

    // Plots for lpt
    c1_must_lpt->cd(bin + 1);
    h1_as_xp_lpt_dat[bin]->Scale(N_inc_sim /
                                 N_inc_dat); // N_dat,tag / N_dat,inc
    label1D_ratio(h1_as_xp_lpt_dat[bin], h1_as_xp_lpt_sim[bin], "Alpha_{S}",
                  "R=R_{tag}/R_{inc}");

    // Plots for hpt
    c1_must_hpt->cd(bin + 1);
    h1_as_xp_hpt_dat[bin]->Scale(N_inc_sim /
                                 N_inc_dat); // N_dat,tag / N_dat,inc
    label1D_ratio(h1_as_xp_hpt_dat[bin], h1_as_xp_hpt_sim[bin], "Alpha_{S}",
                  "R=R_{tag}/R_{inc}");
  }
  ...
  TCanvas *c1_must_xp03_overlaid =
      new TCanvas("c1_must_xp03_overlaid", "", 800, 600);
  ...
  for (int bin = 0; bin < nXp_bins; bin++) {
    ...
    label1D_ratio(h1_as_xp_dat[bin], h1_as_xp_dat[1], "Alpha_{S}",
                  "R=R_{tag}/R_{inc} / R_(x'=0.3)");

    c1_must_xp03_lpt->cd(bin + 1);
    h1_as_xp_lpt_dat[bin]->Divide(h1_as_xp_lpt_sim[bin]);
    label1D_ratio(h1_as_xp_lpt_dat[bin], h1_as_xp_lpt_dat[1], "Alpha_{S}",
                  "R=R_{tag}/R_{inc} / R_(x'=0.3)");

    c1_must_xp03_hpt->cd(bin + 1);
    h1_as_xp_hpt_dat[bin]->Divide(h1_as_xp_hpt_sim[bin]);
    label1D_ratio(h1_as_xp_hpt_dat[bin], h1_as_xp_hpt_dat[1], "Alpha_{S}",
                  "R=R_{tag}/R_{inc} / R_(x'=0.3)");
    ...
    h1_as_xp_lpt_03[bin]->GetYaxis()->SetTitle(
        "R=R_{tag}/R_{inc} / R_(x'=0.3)");
...
void label1D_ratio(TH1D *data, TH1D *sim, TString xlabel, TString ylabel) {
  ...
  //	cout << data_copy->GetXaxis()->GetBinCenter(bin) << " " <<
  // data_copy->GetBinContent(bin) << "\n";
  //	}

  data_copy->Draw("ep");
  TLine *line = new TLine(
      data_copy->GetXaxis()->GetBinCenter(1), 1.,
      data_copy->GetXaxis()->GetBinCenter(data_copy->GetXaxis()->GetNbins()),
      1.);
  line->SetLineWidth(3);
  line->SetLineColor(2);
  line->Draw("same");
